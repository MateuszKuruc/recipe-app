        ...
        <div class="flex flex-1 flex-col gap-8 p-4">
            <div v-if="randomRecipes.length" class="relative w-full overflow-visible px-4">
                <Carousel orientation="horizontal" class="mx-auto w-full max-w-6xl" :opts="{ loop: true }" :plugins="[autoplay]">
                    ...
                    <CarouselNext @click="autoplay.reset()" class="absolute top-1/2 right-3 xl:right-10 z-10 -translate-y-1/2 bg-trsnsparent xl:bg-rose-700 text-white hover:bg-rose-600 hover:text-white" />
                </Carousel>
            </div>
